 ...
 **/
static int
iavf_verify_shaper_info(struct net_device *dev, int nr, const u32 *handles,
			...
			struct netlink_ext_ack *extack)
{
	...
	for (i = 0; i < nr; i++) {
		...
		major = net_shaper_handle_major(handles[i]);
		minor = net_shaper_handle_minor(handles[i]);

		switch (scope)	{
		case NET_SHAPER_SCOPE_QUEUE:
			//Should we need to check if major is same a this VF ??
			if (minor >= adapter->num_active_queues) {
				netdev_err(dev, "Invalid shaper handle at entry %d",
					   i);
				return -EINVAL;
			}
			break;
		...
		case NET_SHAPER_SCOPE_QUEUE_GROUP:
 ...
 **/
static int
iavf_shaper_set(struct net_device *dev, int nr, const u32 *handles,
		const struct net_shaper_info *shapers,
		struct netlink_ext_ack *extack)
{
	struct iavf_adapter *adapter = netdev_priv(dev);
	...
	ret = iavf_verify_shaper_info(dev, nr, handles, shapers, extack);
	if (!ret)
		return ret;
	...
	for (i = 0; i < nr; i++) {
		...
		switch (scope) {
		case NET_SHAPER_SCOPE_QUEUE: {
			struct iavf_ring *tx_ring = &adapter->tx_rings[minor];

			//convert bw into kbps
			tx_ring->q_shaper.bw_min = shapers[i].bw_min;
			tx_ring->q_shaper.bw_max = shapers[i].bw_max;
			tx_ring->q_shaper_update = true;
			if (!adapter->shaper_cfg_update)
				adapter->shaper_cfg_update = true;
		}
			break;
		default:
			ret = -EOPNOTSUPP;
